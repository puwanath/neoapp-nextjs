...
class processController extends Controllers
{
	...
	public function index($get_part0,$get_part1){
		...
		$url = curPageURL();
		$pagemain = "เริ่มระบบ";
		$pagename = "จัดการกระบวนการ (Process)";
		$param = "process";
		$classpage = "";
		$model = new processController;
	...
	public function loadData(){
		...
		$sql = "select count(*) as rowcount from kp_process where is_delete = 0 ";
		if( !empty($requestData['search']) ) {
			$requestData['search']=addslashes($requestData['search']);
			$sql.= "AND  (process_name_th LIKE '%".$requestData['search']."%' ";
			$sql.= "OR  process_name_en LIKE '%".$requestData['search']."%') ";
		}
		$rowcount= $dbCon->query($sql)->fetch_assoc()['rowcount'];
		$start = $rows * ($pageno -1);

		$sql = "select * from kp_process where is_delete = 0 ";
		if( !empty($requestData['search']) ) {
			$requestData['search']=addslashes($requestData['search']);
			$sql.= "AND  (process_name_th LIKE '%".$requestData['search']."%' ";
			$sql.= "OR  process_name_en LIKE '%".$requestData['search']."%') ";
		...
		while($res= $qr->fetch_assoc()){
			...
			$process_name_th = $res['process_name_th'];
			$process_name_en = $res['process_name_en'];


			$created = date("d/m/Y H:i:s",strtotime($res['create_date']));
			$btn_status = "<button type=\"button\" onclick=\"isFuncStatus('".($res['status']==1?0:1)."','{$process_id}')\" class=\"btn btn-sm ".($res['status']==1?'btn-success':'btn-secondary')."\"><i class=\"fa fa-check\"></i></button>";

			$btn_act = array(
				"edit"=>" <button type=\"button\" onclick=\"editFunc('{$process_id}');\" class=\"btn btn-sm btn-warning editform\"><i class=\"icon ion-edit\"></i></button>",
				"del"=>" <button type=\"button\" onclick=\"delFunc('{$process_id}','{$process_name_th}');\" class=\"btn btn-sm btn-danger delitem\"><i class=\"icon ion-trash-a\"></i></button>"
			);
			...
			$row = array();
			$row['id'] = $process_id;
			$row['process_name_th'] = $process_name_th;
			$row['process_name_en'] = $process_name_en;
			$row['created'] = $created;
	...
	{
		$formdata['process_name_th'] = addslashes($_POST['process_name_th']);
		$formdata['process_name_en'] = addslashes($_POST['process_name_en']);

		if($_POST['action']=='add'){
			$formdata['status'] = 1;
			$formdata['is_delete'] = 0;

			$update = $this->insertrow("kp_process",$formdata);
			if($update['status']===TRUE){

				//start function save log transection
				$desclog = "Create process {$_POST['process_name_th']}";
				savelog(getSession(),$desclog);
				//end function save log transection

				$output['status'] = 'success';
				$output['msg'] = 'create process success!';
				$output['id'] = $update['insert_id'];
			}else{
				$output['status'] = 'fail';
				$output['msg'] = 'create process fail!';
				$output['err'] = $update['status'];
			}
			echo json_encode($output);
		}else{
			$process_id = $_POST['process_id'];
			$update = $this->updaterow("kp_process",$formdata,"where process_id = '{$process_id}' ");
			if($update===TRUE){

				//start function save log transection
				$desclog = "Update process {$_POST['process_name_th']}";
				savelog(getSession(),$desclog);
				//end function save log transection

				$output['status'] = 'success';
				$output['msg'] = 'Update process success!';
			}else{
				$output['status'] = 'fail';
				$output['msg'] = 'Update process fail!';
			}
			echo json_encode($output);
		}
	}
	...
	public function loadDataedit(){
		$readData = $_REQUEST;
		$id = $readData['id'];
		$qr = $this->fetchdata("kp_process","where process_id = '{$id}' ");
		$res = $qr->fetch_assoc();

		$output = array();
		$output['process_id'] = $res['process_id'];
		$output['process_name_th'] = $res['process_name_th'];
		$output['process_name_en'] = $res['process_name_en'];
	...
	public function delData()
	{
		...
		$process_name_th = $res->process_name_th;
		if(($qr->num_rows)>0):
		  $del = $this->updaterow("kp_process",array('is_delete'=>'1'),"where process_id = '{$id}' ");
		endif;

		if($del){
			$this->sortDatanew();
			//start function save log transection
			$desclog = "ลบ [ฝ่าย/แผนก] ชื่อ {$process_name_th}";
			savelog(getSession(),$desclog);
			//end function save log transection

			$respon['status'] = "success";
			$respon['msg'] = "Delete [{$process_name_th}] success!";
		}else{
			$respon['status'] = "error";
			$respon['msg'] = "Delete [{$process_name_th}] error!";
		}
	...
	public function changeIsstatus(){
		...
		$process_name_th = $this->getTextformid("kp_process","process_name_th","process_id",$id);

		if($id){
			$updatestatus = $this->updaterow("kp_process",array('status'=>$status),"where process_id = '{$id}' ");
		}


		if($updatestatus){
			//start function save log transection
			$desclog = "เปลี่ยน $process_name_th ให้เปิดใช้งาน สำเร็จ!";
			savelog(getSession(),$desclog);
			//end function save log transection

			$respon['status'] = "success";
			$respon['msg'] = "Change Status [$process_name_th] success!";
		}else{
			$respon['status'] = "success";
			$respon['msg'] = "Change Status [$process_name_th] error!";
		}
		echo json_encode($respon);
	}

	public function sortDatanew(){
		$qr = $this->fetchdata("kp_process","where is_delete=0 order by sort asc");
		...
		foreach ($arrid as $value) {
			$process_id = $value;
			$data = array('sort'=>$i);
			$this->updaterow("kp_process",$data,"where process_id = '".$process_id."' ");
			//echo "<br/>$i<br/>";
			$i++;
	...
	public function sortRow(){
		...
		if($_REQUEST['line']==0 and $sorttype=="up"){
		...
		}elseif($_REQUEST['line']==($this->countdatasort("sort","kp_process")-1) and $sorttype=="down"){
			$output['status'] = "error";
			...
			if($sorttype=="up"){
				...
				$sort_plus = $sort+1;
				$sort_post = $sort-1;
				$qr = $this->fetchdata("kp_process","where sort = '$sort' ");
				$resu = $qr->fetch_object();

				$qr2 = $this->fetchdata("kp_process","where sort = '$sort_post' ");
				...
				$this->updaterow("kp_process",$sortdata,"where process_id = '".$resu->process_id."' ");
				// echo "track = $track\n";
				...
				$this->updaterow("kp_process",$sortdata2,"where process_id = '".$resu2->process_id."' ");
				...
				$qr2 = $this->fetchdata("kp_process","where sort = '$sort_plus' ");
				$resu2 = $qr2->fetch_object();
