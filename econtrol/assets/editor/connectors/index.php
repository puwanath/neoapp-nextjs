...
//$root = '../../../images/';
$root = realpath('../../../../images/').DIRECTORY_SEPARATOR;
$relpath = isset($_REQUEST['path']) ?  $_REQUEST['path'] : '';
$baseurl = isset($_REQUEST['baseurl']) ?  $_REQUEST['baseurl'] : '';
$path = realpath($root).DIRECTORY_SEPARATOR;
if (realpath($path.$relpath) && strpos(realpath($path.$relpath), $root) !== false) {
...
if (isset($_FILES['images']) 
    ...
    foreach ($_FILES['images']['name'] as $i=>$file) {
        ...
        if (move_uploaded_file($tmp_name, $file = $path.makeSafe($_FILES['images']['name'][$i]))) {
            ...
            $result->files[] = str_replace($root, '', $file);
            //$result->baseurl[] = $base2;
            $result->path = str_replace(realpath($root) . DIRECTORY_SEPARATOR, '', $path);
