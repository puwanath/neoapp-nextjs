...
class Controllers
{
	...
	public function countDataEventPackage($event_id,$package_id){
		require 'common/include/config.php';
		$sql = "select count(*) as countnum from kp_events_runners where event_id = '$event_id' and package_id = '$package_id' and status != 'cancel' ";
		$qr = $dbCon->query($sql) or die($dbCon->error);
		$res = $qr->fetch_object();
	...
	public function countDatarunner($event_id){
		require 'common/include/config.php';
		$sql = "select count(*) as countnum from kp_events_runners where event_id = '$event_id' and status != 'cancel' ";
		$qr = $dbCon->query($sql) or die($dbCon->error);
		$res = $qr->fetch_object();
	...
	public function countDataevent($event_id){
		require 'common/include/config.php';
		$sql = "select sum(event_package_qty) as sumqty from kp_events_package where event_id = '$event_id' and status=1 and is_delete=0 ";
		$qr = $dbCon->query($sql) or die($dbCon->error);
		$res = $qr->fetch_object();
