  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content tx-size-sm">
                ...
                <div class="row no-gutters">
                  <div class="col-md-12 mg-t--1 mg-md-t-0">
                    <div class="form-group bd-t-0-force bd-l-0-force bd-r-0-force">
                      <label class="form-control-label">ธนาคาร: <span class="tx-danger">*</span></label>
                      <input class="form-control" type="text" name="bank_name" id="bank_name" value="" placeholder="...">
                      <span id="bank_name_msg" class="text-danger"></span>
                    </div>
                  </div>
                  <div class="col-md-12 mg-t--1 mg-md-t-0">
                    <div class="form-group bd-t-0-force bd-l-0-force bd-r-0-force">
                      <label class="form-control-label">สาขา: <span class="tx-danger">*</span></label>
                      <input class="form-control" type="text" name="bank_branch" id="bank_branch" value="" placeholder="...">
                      <span id="bank_branch_msg" class="text-danger"></span>
                    </div>
                  </div>
                  <div class="col-md-12 mg-t--1 mg-md-t-0">
                    <div class="form-group bd-t-0-force bd-l-0-force bd-r-0-force">
                      <label class="form-control-label">ชื่อบัญชี: <span class="tx-danger">*</span></label>
                      <input class="form-control" type="text" name="bank_acc_name" id="bank_acc_name" value="" placeholder="...">
                      <span id="bank_acc_name_msg" class="text-danger"></span>
                    </div>
                  </div>
                  <div class="col-md-12 mg-t--1 mg-md-t-0">
                    <div class="form-group bd-t-0-force bd-l-0-force bd-r-0-force">
                      <label class="form-control-label">เลขบัญชี: <span class="tx-danger">*</span></label>
                      <input class="form-control" type="text" name="bank_acc_num" id="bank_acc_num" value="" placeholder="...">
                      <span id="bank_acc_num_msg" class="text-danger"></span>
                    </div>
                  </div>
      ...
      <form name="frmupload" method="post" action="" id="fle-form" enctype="multipart/form-data">
				<input type="hidden" name="bank_id" id="id" />
  ...
  $(document).ready(function () {
    ...
    $("#fle-form").on('submit',(function(e){
      ...
      $("#addform").on('click',function(e){
        e.preventDefault();
        $(".titleform").text('เพิ่มบัญชีธนาคาร');
        $("#formdata")[0].reset();
        $("#fle-form")[0].reset();
        $("#action").empty();
        $("#action").val('add');
      ...
      $(".btnsave").on('click',function(e){
        e.preventDefault();
				...
				if($("#bank_name").val()==""){
					$('#bank_name_msg').text('กรุณากรอกช่องนี้ด้วยครับ');
				}else{
          $('#bank_name_msg').text('');
        }
				if($("#bank_branch").val()==""){
					$('#bank_branch_msg').text('กรุณากรอกช่องนี้ด้วยครับ');
				}else{
          $('#bank_branch_msg').text('');
        }
				if($("#bank_acc_name").val()==""){
					$('#bank_acc_name_msg').text('กรุณากรอกช่องนี้ด้วยครับ');
				}else{
          $('#bank_acc_name_msg').text('');
        }
				if($("#bank_acc_num").val()==""){
					$('#bank_acc_num_msg').text('กรุณากรอกช่องนี้ด้วยครับ');
				}else{
          $('#bank_acc_num_msg').text('');
        }

				if($("#bank_name").val()!='' && $("#bank_branch").val()!='' && $("#bank_acc_name").val()!='' && $("#bank_acc_num").val()!=''){
					$("#formdata").submit();
				}else{
					return false;
				}
    ...
    function editFunc(id){
      $(".titleform").text('แก้ไขบัญชีธนาคาร');
      $("#modalform").modal('show');
      $("#formdata")[0].reset();
      $("#fle-form")[0].reset();
      $("#action").empty();
