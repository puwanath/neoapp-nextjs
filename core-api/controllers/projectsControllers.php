...
class projectsController extends Controllers
{
	public function index($get_part0,$get_part1,$get_part2,$get_part3){
		...
		$pagename = $word->wordvar("Projects");
		$bgcover = "$url/images/bgabout.jpg";
		$param = "projects";
		$active_allproject = "c-active";
		$model = new projectsController;


		if($get_part0!='projects' and $get_part0!=''){
			putrating("kp_projects_cat","slug",$get_part0); //update static
			...
			$pagename = $word->wordvar("All Projects");
	...
	public function listProjectscat(){
		$word = new word();
		$url = curPageURL();
		$arr = array();
		$qr = $this->fetchdata('kp_projects_cat',"where status = 1 and is_delete=0 order by sort asc");
	...
	public function getData($fill,$id){
		$url = curPageURL();
		$word = new word();
		$qr = $this->fetchdata("kp_projects_cat","where slug = '$id' ");
		$res = $qr->fetch_object();
		$project_cat_id = $res->project_cat_id;
		...
		if($fill=="logo"){
			if($res->project_cat_logo!=''){
				if(file_exists("images/projects_cat/$res->project_cat_logo")==true){
					$img_logo = img_webp("images/projects_cat/$res->project_cat_logo");
					return "<img src='$img_logo' class='img-fluid' style='height:100px;' />";
				}
			}
		}elseif($fill=="cover"){
			...
			if($getdevice=="Computer"){
				$bg_cover = $this->getTextfromwhere('kp_projects_cat_gallery','project_cat_img',"where project_cat_id = '$project_cat_id' and is_desktop=1 and img_type='cover' ");
			}elseif($getdevice=="Tablet"){
				$bg_cover = $this->getTextfromwhere('kp_projects_cat_gallery','project_cat_img',"where project_cat_id = '$project_cat_id' and is_tablet=1 and img_type='cover' ");
			}elseif($getdevice=="Mobile"){
				$bg_cover = $this->getTextfromwhere('kp_projects_cat_gallery','project_cat_img',"where project_cat_id = '$project_cat_id' and is_mobile=1 and img_type='cover' ");
			}else{
				$bg_cover = $this->getTextfromwhere('kp_projects_cat_gallery','project_cat_img',"where project_cat_id = '$project_cat_id' and is_desktop=1 and img_type='cover' ");
			}
			if(!empty($bg_cover)){
				return img_webp("images/projects_cat/$bg_cover");
			}else{
				return img_webp("images/banner1440.jpg");
			}

		}elseif($fill=="cover-img"){
			$img=  $this->getImgcoverCat($project_cat_id);
			return '<img src="'.$img.'" class="img-responsive" style="margin:auto;"/>';
		}elseif($fill=="name"){
			if($_SESSION['lg']=='TH'){
				return $res->project_cat_name_th;
			}else{
				return $res->project_cat_name_en;
			}
		}elseif($fill=="detail"){
			if($_SESSION['lg']=='TH'){
				return $res->project_cat_detail_th;
			}else{
				return $res->project_cat_detail_en;
			}
		}elseif($fill=="shortdetail"){
			if($_SESSION['lg']=='TH'){
				return $res->project_cat_shortdetail_th;
			}else{
				return $res->project_cat_shortdetail_en;
			}
		}elseif($fill=="concept"){
			if($_SESSION['lg']=='TH'){
				return $res->project_cat_concept_desc_th;
			}else{
				return $res->project_cat_concept_desc_en;
			}
		}elseif($fill=="concept_img"){
			if($res->project_cat_concept_img!=''){
				if(file_exists("images/projects_cat/$res->project_cat_concept_img")==true){
					$img_concept = img_webp("images/projects_cat/$res->project_cat_concept_img");
					return "<img src='$img_concept' class='img-responsive' style='width:100%;' />";
				}
			}
		}elseif($fill=="map"){
			return $res->project_cat_map;
		}elseif($fill=="gallery"){
			$arr = array();
			$qr = $this->fetchdata("kp_projects_cat_gallery","where project_cat_id = '$project_cat_id' and img_type='gallery' and is_cover!=1 order by sort asc ");
			while($res = $qr->fetch_object()){
				if(file_exists("images/projects_cat/$res->project_cat_img")){
					$img_gall = img_webp("images/projects_cat/$res->project_cat_img");
					$img = $img_gall;

					// $data = "<div class=\"col-md-4\">
	        //   <div class=\"card mb-4 hoverable\">
	        //     <div class=\"view\" style=\"max-height: 200px;\">
	        //       <a href=\"$img\" class=\"tt-btn-zomm\" title=\"$project_cat_name\"><img class=\"card-img-top\" style=\"width: 100%; display: block;\" alt=\"$project_cat_name\" src=\"$img\" data-holder-rendered=\"true\"></a>
	        //     </div>
	        //   </div>
	        // </div>";
					$data = "<div >
			...
			if(count($arr)>=3){
				// $datahtml.= "<div id=\"links\"  class=\"slider multiple-items-gallery row\">";
	...
	public function loadDatahomeplan(){
		require 'econtrol/common/include/config.php';
		$url = curPageURL();
		$word = new word();
		$arr = array();
		$readData = $_GET;
		$project_cat_id = $readData['project_cat_id'];
		// $type = $readData['type'];
		...
		$sql = "select * from kp_projects ";
		$sql.= "where is_delete=0 and status = 1 and project_cat_id = '$project_cat_id' ";
		$sql.= "order by sort asc limit $limit0,$limit1";
		...
		while($res = $qr->fetch_object()){
			...
			$imgcover = $this->getImgcover($project_id);

			if($res->is_soldout==1){
				$imgsoldout = img_webp("images/soldout.png");
				$imgsoldout = "<img src=\"$imgsoldout\" class=\"img-responsive img-soldout\" style=\"height:100%;width:100%;\" alt=\"\">";
			}else{
				$imgsoldout = "";
			}
	...
	public function getDataproject($fill,$id){
		$url = curPageURL();
		$word = new word();
		$qr = $this->fetchdata("kp_projects","where project_id = '$id' ");
		$res = $qr->fetch_object();
		...
		if($fill=="logo"){
			if($res->project_logo!=''){
				if(file_exists("images/projects/$res->project_logo")==true){
					...
					return "<img src='$img_logo' class='img-fluid' />";
				}
			}
		}elseif($fill=="name"){
			if($_SESSION['lg']=='TH'){
			...
			$arr = array();
			$qr = $this->fetchdata("kp_projects_gallery","where project_id = '$id' and img_type = 'gallery' order by sort asc ");
			while($res = $qr->fetch_object()){
			...
			$arr = array();
			$qr = $this->fetchdata("kp_projects_gallery","where project_id = '$id' and img_type = 'plan' order by sort asc ");
			while($res = $qr->fetch_object()){
				if(file_exists("images/projects/$res->project_img")){
					$img = img_webp("images/projects/$res->project_img");
	          ...
	          <div class=\"card mb-4 hoverable\">
	            <div class=\"view\" >
	              <a href=\"#\" ><img class=\"card-img-top\" style=\"width: 100%; display: block;\" alt=\"$project_name\" src=\"$img\" data-holder-rendered=\"true\"></a>
	            </div>
	          </div>
	        </div>";
					array_push($arr,$data);
			...
			if(count($arr)>0){
				$datahtml = "<div class=\"tt-block-title text-left\" style=\"padding-bottom:20px;\">";
					$datahtml.= "<h1 class=\"tt-title project-headblog\">".$word->wordvar('Home Plans')."</h1>";
				$datahtml.= "</div>";
...
'fullscreen=yes, scrollbars=auto');\"><img src=\"$url/images/360view.jpg\" class=\"img-fluid\" style=\"max-height:412px;\" alt=\"$project_name\" /></a>";
			...
			$arr = array();
			$qr = $this->fetchdata("kp_projects_detail","where project_id = '$id' order by text_sort asc ");
			while($res = $qr->fetch_object()){
