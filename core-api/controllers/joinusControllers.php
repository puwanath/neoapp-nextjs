...
class joinusController extends Controllers
{
	public function index($get_part0,$get_part1,$get_part2,$get_part3){
		...
		$pagename = $word->wordvar("Join us");
		$bgcover = "$url/images/bgabout.jpg";
		$param = "joinus";
		$model = new joinusController;
	...
	public function getMenulist($id){
		...
		while($res = $qr->fetch_object()){
			...
			$data = "<li class=\"$active\"><a href=\"$url/joinus/$res->slang\"><i class=\"icon icon-cursor\"></i> $pagename </a></li>";
			array_push($arr,$data);
		}
		return $arr;
	}
